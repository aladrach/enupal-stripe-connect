...
class Checkout extends Component
{
    ...
    private function handleOneTimePayment(PaymentForm $paymentForm, $postData, $metadata, $sessionParams)
    {
        ...
        if ($paymentForm->stripeId && $paymentForm->platformFee ) {
            $sessionParams['payment_intent_data'] = [
                'metadata' => $metadata,
                'application_fee_amount' => ceil($data['amount'] * ($paymentForm->platformFee * .01) + 25),
                "transfer_data" => [
                    "destination" => $paymentForm->stripeId,
                  ],
            ];
        }
